...
#if defined(ANDROID) && defined(RTLSDR)
/* Android-only: open RTL-SDR by a pre-obtained USB file descriptor.
 * Uses rtlsdr_open2() from the rtl_tcp_andro patched librtlsdr,
 * which wraps libusb_open2() to accept an already-opened fd. */
extern int rtlsdr_open2(rtlsdr_dev_t **out_dev, int fd, const char *device_path);

static int sdr_open_rtl_fd(sdr_dev_t **out_dev, int fd, const char *device_path, int verbose)
{
    sdr_dev_t *dev = calloc(1, sizeof(sdr_dev_t));
    if (!dev) {
        WARN_CALLOC("sdr_open_rtl_fd()");
        return -1;
    }
#ifdef THREADS
    pthread_mutex_init(&dev->lock, NULL);
    ...
    if (verbose)
        print_logf(LOG_NOTICE, "SDR", "Opening RTL-SDR native USB fd=%d path=%s", fd, device_path);

    int r = rtlsdr_open2(&dev->rtlsdr_dev, fd, device_path);
    if (r < 0) {
        if (verbose)
            print_logf(LOG_ERROR, __func__, "rtlsdr_open2 failed: %d", r);
        free(dev);
        return r;
    }
...
#if defined(ANDROID) && defined(RTLSDR)
    /* Android native USB: dev_query = "fd:N:device_path" */
    if (dev_query && !strncmp(dev_query, "fd:", 3)) {
        const char *fd_str = dev_query + 3;
        int fd = (int)strtol(fd_str, NULL, 10);
        const char *sep = strchr(fd_str, ':');
        return sdr_open_rtl_fd(out_dev, fd, sep ? sep + 1 : "", verbose);
    }
#endif
